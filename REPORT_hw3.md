...
**По точности:**
- **Алгоритм**: SVM с линейным ядром
- **Эксперимент**: SVM_linear
- **Test Accuracy**: 1.000 (100%)
- **F1-score**: 1.000
- **CV Accuracy**: 0.8498 ± 0.0630

**По алгоритмам:**
- **SVM**: Средняя точность 0.8364, лучшая 1.000
- **Random Forest**: Средняя точность 0.9091, стабильные результаты
- **Logistic Regression**: Средняя точность 0.8727, лучшая 0.9091
...
3. **Воспроизведение всех экспериментов:**
   ```bash
   python scripts/run_experiments.py
   ```
...
4. **Просмотр результатов:**
   ...
   - Детальные результаты: папка `experiments/`

**Примерное время выполнения:** ~2-3 минуты для всех 17 экспериментов
